...
const SurvivalMode: React.FC = () => {
  ...
  useEffect(() => {
    const sorted = [...BASE_QUESTIONS].sort((a, b) => a.difficulty - b.difficulty);
    setQuestions(sorted);
    setIndex(0);
    setScore(0);
  ...
  const onSelect = (optIndex: number) => {
    ...
    if (optIndex === q.correctIndex) {
      ...
      if (nextIdx >= questions.length) {
        ...
        setIndex(questions.length - 3 >= 0 ? questions.length - 3 : questions.length - 1);
      } else {
        setIndex(nextIdx);
      }
      setTimeLeft(PER_QUESTION_TIME);
    ...
    <div className="min-h-screen w-full relative overflow-hidden bg-background">
      ...
      <header className="relative z-10 w-full px-4 sm:px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="px-2 py-1 rounded-md text-xs font-bold border border-red-500/60 text-red-400 uppercase">1 Life Only</span>
        </div>
        <div className={`px-3 py-1 rounded-lg font-mono text-sm border ${timeLeft <= 3 ? "border-red-500/60 text-red-400" : "border-electric-500/50 text-electric-400"}`}>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center px-4 sm:px-8 pt-2 pb-24">
        <div className="max-w-3xl w-full bg-card/70 backdrop-blur-md rounded-2xl border border-border p-6 shadow-xl">
          ...
          {q ? (
            <div>
              <p className="text-lg sm:text-xl font-semibold mb-6 text-foreground">{q.prompt}</p>
      ...
      {gameOver && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-background/90 backdrop-blur-xl">
          <div className="w-full max-w-md mx-auto bg-card rounded-2xl border border-border p-6 text-center shadow-2xl">
            <h2 className="text-2xl font-extrabold mb-2 text-red-500">Game Over</h2>
            <p className="text-muted-foreground">Final Score: <span className="font-bold text-cyan-500">{score}</span></p>
            <p className="mt-1 text-sm text-foreground">Highest Score: <span className="font-bold text-yellow-400">{best}</span></p>
            <div className="mt-6 flex items-center justify-center gap-3">
              <Button onClick={handleRetry} className="bg-gradient-to-r from-red-500 to-electric-500 text-white">Retry</Button>
